...
void next_wif(){
  ...
  if (!SD.exists(path)){
    // assume over shot last chapter. start over
    current_wif = 0;
  }
}
...
void next_dir(){
  current_dir++;
  get_cwd_path(); // copy new dir path to path
  if (!SD.exists(path)){
    current_dir = 0; // assume over shot last chapter. start over
  }
  open_cwd();
...
void prev_dir(){
  bool done;
  current_dir--;
  if(current_dir < 0){ // go to very last dir
    done = false;
    for(current_dir=0; !done; current_dir++){
      get_cwd_path(); // copy new dir path to path
      if(!SD.exists(path)){
	// back one to get last... back another for current_dir++ in for(;;++)
	current_dir -= 2; 
	get_cwd_path(); // copy new dir path to path
	done = true;
      }
...
void num_to_txt(int num, char* txt){
  num %= 10000;
...
void get_wif_path(){
  ...
  if(!SD.exists(path)){
    ...
    ereader.put_ascii(10, 60, path, true);
    ereader.show();
  }
}
